...
\begin{figure}
\centering
\includegraphics[height=15em]{system2.png}
...
\EndFor
\State $w_{new} \gets Finalization($state$)$ %\Comment{Invoke L-BFGS convex solver}
\Until{$Convergence(w_{new}, g_{new}, iteration)$}
    \State \Return $w_{new}$
\end{algorithmic}
...
\begin{algorithmic}[1]
        ...
        \State state $\gets$ instance.state \Comment{Get Updated state}
        \State \Return state
\end{algorithmic}
\label{algo:crffinal}
\end{algorithm}
...
In the future we plan to support more text analytic functions 
such as parallel Markov Chain Monte Carlo sampling 
for entity resolution in parallel RDBMS.
